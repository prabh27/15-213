...
void eval(char *cmdline) 
{
    ...
    if (!builtin_cmd(argv)) {
        ...
        if (pid == 0) {    /* Child runs user job */
            setpgid(0, 0);
            sigprocmask(SIG_SETMASK, &prev_one, NULL);  /* Unblock SIGCHILD for child */
...
void sigchld_handler(int sig) 
{
    ...
    sigfillset(&mask_all);
    while ((pid = waitpid(getpgrp(), NULL, 0)) > 0) {
